            ...
            <div className="flex flex-col justify-center h-screen">
                <div className=" bg-slate-50 border-2 border-slate-400 p-10 rounded-lg">
                    <div className="text-center mb-10">
                        <h1 className="text-4xl font-bold text-sky-400">Create Room</h1>
                    ...
                    <div className="mt-8">
                        <button onClick={async ()=>{
                            ...
                            await localStorage.setItem("action", "create_room");
                            await navigate('/room');
                        }} className="bg-sky-400 text-slate-50 p-2 max-w-md w-full rounded-md text-lg font-semibold">Enter</button>
                    </div>
                    <div className="text-center mt-4">
                       <span className="font-medium text-slate-400">join room instead? </span> <Link  to={'/join-room'} className="text-md font-bold text-slate-400 hover:text-slate-900 underline">Join</Link>
                    </div>
                </div>
            </div>
        </div>
